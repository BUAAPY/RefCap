...
@REGISTER_RETRIEVEPIPE(["key"])
class KeyInferPipe(BaseRetrievePipe):
    ...
    def retrieval(self, test_dataset: DataSet4Test):
        ...
        for idx, batch in tqdm(enumerate(dataloader), desc="Evaluating...", total=len(dataloader)):
            ...
            """
            descs_keys = list(map(lambda x: tree_utils.extract_keys(self.nlp, self.captree.key_feature_model.stoi, x), descs)) # List[List[str]]
            ...
            S_perquery_maxevent = S_perquery_perevent.max(dim=3)[0]
            ...
            esm_sims = key_sims_per_proposal
            ...
            key_scores = sim_utils.normalize_min_max(key_scores, dim=1) 
            scores = key_scores 
